...
class DetailedMetricsVisualizer:
    ...
    def _plot_field(self, ax, field_data, ego, others, title, occluder_ids):
        ...
        ax.contour(X, Y, R_norm, levels=levels, colors='white', alpha=0.3, linewidths=0.5)
        ...
        for other in others:
            ...
            rect = mpatches.FancyBboxPatch(
                ...
                boxstyle="round,pad=0.02",
                facecolor=color, edgecolor='white', linewidth=1.5 if is_occ else 1, alpha=0.85
            )
            ax.add_patch(rect)
    ...
    def _plot_field_with_occlusion(self, ax, X, Y, R_full, R_occ, shadow_mask,
        ...
        ego_rect = mpatches.FancyBboxPatch(
            (-ego.get('length', 5)/2, -ego.get('width', 2)/2),
            ego.get('length', 5), ego.get('width', 2),
            boxstyle="round,pad=0.02", facecolor='#E74C3C', edgecolor='white', linewidth=2
        )
        ax.add_patch(ego_rect)
    ...
    def _plot_boundary_bars(self, ax, metrics, methods):
        ...
        for i, method in enumerate(methods):
            ...
            ax.bar(x + i * width - width * len(methods) / 2 + width / 2,
                  values, width, label=method.upper(), color=colors[i], edgecolor='white')
...
def main(data_dir: str, recording_id: int, ego_id: Optional[int] = None,
         ...
         methods: List[str] = None, light_theme: bool = False):
    ...
    output_file = output_path / f'metrics_rec{recording_id}_ego{ego_id}_frame{frame}.png'
    visualizer.create_detailed_figure(snapshot, metrics, output_path=str(output_file))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--light-theme', action='store_true', help='Use light theme')
    
    args = parser.parse_args()
    
    main(args.data_dir, args.recording, args.ego_id, args.frame, 
         args.output_dir, args.methods, args.light_theme)

...
class InteractionFieldAnalyzer:
    ...
    def _compute_shadows(self, ego: VehicleState, 
        ...
        """Compute shadow polygons using tangents from ego front to heavy occluders."""
        def vehicle_corners(v: VehicleState) -> np.ndarray:
            c = np.array([v.x, v.y])
            hl, hw = v.length / 2.0, v.width / 2.0
            offsets = np.array([
                [ hl,  hw],
                [ hl, -hw],
                [-hl, -hw],
            ...
            rot = np.array([[np.cos(v.heading), -np.sin(v.heading)],
                            [np.sin(v.heading),  np.cos(v.heading)]])
            return (rot @ offsets.T).T + c
        
        shadows = []
        # Only consider heavy vehicles as occluders, select nearest in front
        cos_h = np.cos(ego.heading)
        sin_h = np.sin(ego.heading)
        ego_front = np.array([
            ego.x + (ego.length / 2.0) * cos_h,
            ego.y + (ego.length / 2.0) * sin_h
        ])
        
        heavy = [
            v for v in surrounding
            if v.vehicle_class in self.config.HEAVY_VEHICLE_CLASSES
            and ( (v.x - ego.x) * cos_h + (v.y - ego.y) * sin_h ) > 0.0
        ]
        if not heavy:
            return shadows
        
        # For robustness, sort by distance from ego front and handle each heavy occluder
        heavy.sort(key=lambda v: np.hypot(v.x - ego_front[0], v.y - ego_front[1]))
        
        for occluder in heavy:
            corners = vehicle_corners(occluder)
            # If occluder is not heavy enough (e.g., passenger car), skip tangents
            occluded_ids = [v.id for v in surrounding if v.is_occluded and v.occluder_id == occluder.id]
            if not occluded_ids:
                continue
            
            # Angles from ego front to each corner
            angles = np.array([np.arctan2(c[1] - ego_front[1], c[0] - ego_front[0]) for c in corners])
            left_idx = int(np.argmax(angles))
            right_idx = int(np.argmin(angles))
            left_corner = corners[left_idx]
            right_corner = corners[right_idx]
            
            far = self.config.OCCLUSION_RANGE
            left_far = ego_front + far * np.array([np.cos(angles[left_idx]), np.sin(angles[left_idx])])
            right_far = ego_front + far * np.array([np.cos(angles[right_idx]), np.sin(angles[right_idx])])
            ...
            shadows.append(OcclusionShadow(
                occluder_id=occluder.id,
                occluded_ids=occluded_ids,
                polygon=polygon,
                cone_near=np.linalg.norm(left_corner - ego_front),
                cone_far=far
            ))
    ...
    def _draw_road_layout(self, ax, ego: VehicleState, X, Y, draw_fill: bool = True):
        ...
        Background image is reprojected into the ego frame (ego at origin, heading along +X)."""
        if self.loader and self.loader.background_image is not None and self.loader.background_extent is not None:
            ...
            # Transform corners into ego frame
            dxdy = corners - np.array([ego.x, ego.y])
            rot = np.array([[np.cos(-ego.heading), -np.sin(-ego.heading)],
                            [np.sin(-ego.heading),  np.cos(-ego.heading)]])
            ego_corners = (rot @ dxdy.T).T
            ex_min, ey_min = ego_corners.min(axis=0)
            ex_max, ey_max = ego_corners.max(axis=0)
            ax.imshow(self.loader.background_image,
                      extent=(ex_min, ex_max, ey_min, ey_max),
                      origin='lower', alpha=0.6, zorder=0, aspect='equal')
            return
